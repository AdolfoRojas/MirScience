...
process Dowload_SRA_info { 
    ...
    df5.to_excel(Disease+"_Biosample.xlsx", index = False)
    df5.to_csv(Disease+"_Biosample.tsv",sep = "\t", index = False)


    df8 = df4.merge(df5,left_on="BiosampleID",right_on="Accession")
    df8.to_excel(Disease+"_Biosample_SRA.xlsx", index = False)
    df8.to_csv(Disease+"_Biosample_SRA.tsv",sep = "\t", index = False)
    ...
    df8.to_csv(Disease+"_samples_SRA.csv",sep = ",", index = False)
    """   
}
...
workflow { 
    params.outdir = "Resultados/Datasets/${params.Disease_term}/"
    ...
    Dowload_SRA_info(Dowload_Bioproject_info.out)
    //Dowload_GEO_info(Dowload_SRA_info.out)
}