...
process Dowload_fastqs {   
    ...
    script:
    """
    fastq-dump --split-files --gzip ${run_acc}
    """
}
process FASTQC {
...
process Generate_out_dir {      

    input:
    tuple val(Organism), val(Library_Layout), val(Project), val(Platform)
    ...
    script:
    """    
    echo Resultados/${Organism}/${Library_Layout}/${Project}/
    """
}
process FASTP {    //
    maxForks 5    
...
process strandness_test { 
    ...
    script:
    """
    check_strandedness -g /media/storage/datasets/gtf/mouse/gencode.vM27.annotation.gtf --transcripts /media/storage/Adolfo/MirScience/gencode.vM27.transcripts.fa -r1 ${reads[0]} -r2 ${reads[1]} | tail -1 | cut -d' ' -f 4| cut -d'/' -f 1 > detected_strandedness.txt
    """
}
process Hisat2 {    //
    maxForks 2
    ...
    if (strandness == "RF")
        ...
        """
        hisat2 --rna-strandness FR -p $task.cpus --summary-file '$sample_id'.summary -x /media/storage/datasets/indexes/hisat2_index/Mus_musculus/Mm_GRCm39/GRCm39  -1 ${reads[0]} -2 ${reads[1]} -S '$sample_id'.sam
        java -jar /media/storage2/software/Picard/picard.jar SortSam VALIDATION_STRINGENCY=SILENT SORT_ORDER=coordinate INPUT= '$sample_id'.sam OUTPUT= '$sample_id'.sorted.bam
        """
    else if (strandness == "unstranded")
        """
        hisat2 -p $task.cpus --summary-file '$sample_id'.summary -x /media/storage/datasets/indexes/hisat2_index/Mus_musculus/Mm_GRCm39/GRCm39  -1 ${reads[0]} -2 ${reads[1]} -S '$sample_id'.sam
        java -jar /media/storage2/software/Picard/picard.jar SortSam VALIDATION_STRINGENCY=SILENT SORT_ORDER=coordinate INPUT= '$sample_id'.sam OUTPUT= '$sample_id'.sorted.bam
        """
    else
...
workflow { 
    ...
    params.Platform = "illumina_hiseq_2500"
    ...
    if (params.Organism == "Mus_musculus"){
        ...
        params.Annotation = "../IDs_equivalencias_gencode.vM27.txt"
        ...
        params.Annotation = "../IDs_equivalencias_gencode.v38.txt"
    ...
    print "Annotation file: " + params.Annotation
    Dowload_fastqs(Runs)
    QC1(Dowload_fastqs.out.fastqs)
    Trimm_QC2(Dowload_fastqs.out.fastqs)
    Map_and_count(Trimm_QC2.out)  // no inicio en la primera  
