...
process FASTP {    //
    ...
    if (params.Library_Layout == "PAIRED")    
        ...
        """
        fastp --thread $task.cpus $params.mod_FastP --in1 ${reads[0]} --length_required 40 --detect_adapter_for_pe --cut_right --cut_right_window_size 4 --cut_right_mean_quality 30 --trim_tail1=5 --trim_front1=15 --out1 '$sample_id'_trimmed_R1.fastq.gz
        """
    else
...
process Hisat2 {    //
    ...
    if (params.Library_Layout == "PAIRED") 
        ...
        if (strandness == "RF")
            """
            hisat2 --rna-strandness RF -p $task.cpus --summary-file '$sample_id'.summary -x $params.index_genome -U ${reads[0]} -S '$sample_id'.sam
            java -jar /media/storage2/software/Picard/picard.jar SortSam VALIDATION_STRINGENCY=SILENT SORT_ORDER=coordinate INPUT= '$sample_id'.sam OUTPUT= '$sample_id'.sorted.bam
            """
        else if (strandness == "FR")
            """
            hisat2 --rna-strandness FR -p $task.cpus --summary-file '$sample_id'.summary -x $params.index_genome -U ${reads[0]} -S '$sample_id'.sam
            java -jar /media/storage2/software/Picard/picard.jar SortSam VALIDATION_STRINGENCY=SILENT SORT_ORDER=coordinate INPUT= '$sample_id'.sam OUTPUT= '$sample_id'.sorted.bam
            """
        else if (strandness == "unstranded")
            """
